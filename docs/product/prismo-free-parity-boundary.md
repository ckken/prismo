...
| Components | 可发现、可组合、可验证的基础和视觉组件 | 可复用官方 shadcn 原语，但 Prismo 维护自身 Variant、组合和验收契约 |
...
Prismo 不实现或模拟：价格页、订阅、Trial、额度、License、用量限制、优先支持、付费账户管理或付款处理。
...
一个 Prismo 请求只有同时满足以下条件才是 `complete`：
